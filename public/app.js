...
async function checkMe(){
  ...
  if(user){$('auth').classList.add('hidden');$('app').classList.remove('hidden');$('userBox').innerHTML=`<span>已登录：${escapeHtml(user.username)}</span><button class="secondary" onclick="logout()">退出</button>`;loadArticles();}
...
async function loadArticles(){const data=await request('/api/articles');articles=data.articles;$('articleList').innerHTML=articles.map(a=>`<div class="item" onclick="openArticle(${a.id})"><strong>${escapeHtml(a.title)}</strong><span>${escapeHtml(a.disease)} · ${escapeHtml(a.updated_at)}</span></div>`).join('')||'<p class="hint">暂无文章，点“新建”开始。</p>'; if(!articles.length)newArticle();}
